...
QtObject {
    ...
    /* Theme accessors, wrapped.

       tcolor()/num()/str()/flag() are invokable methods. A QML binding
       whose only dependency is a method call is evaluated once and never
       again — there is no property read for the engine to record — so every
       themed value below was frozen at startup. Editing a theme file logged
       "theme loaded" and changed precisely nothing on screen, which the README
       promised it would.

       Reading Theme.revision here is the fix, and the only reason it is read:
       the engine records it while evaluating the binding, so a reload
       re-evaluates every value that came through these. */
    function tcolor(key, fallback) { const r = Theme.revision; return Theme.color(key, fallback) }
    function tnum(key, fallback)   { const r = Theme.revision; return Theme.num(key, fallback) }
    function tstr(key, fallback)   { const r = Theme.revision; return Theme.str(key, fallback) }
    function tflag(key, fallback)  { const r = Theme.revision; return Theme.flag(key, fallback) }
    ...
    // ---- Geometry ------------------------------------------------------
    readonly property int padding: Math.round(tnum("spacing.padding", px(12)))
    ...
    readonly property int pillPaddingH: Math.round(tnum("spacing.pillPaddingH", px(20)))
    readonly property int spacing: Math.round(tnum("spacing.gap", px(13)))
    readonly property int iconSize: Math.round(tnum("size.icon", px(21)))
    ...
    readonly property real chipRadiusRatio: tnum("radius.chipRatio", 0.32)
    ...
    readonly property int minWidth: Math.round(tnum("size.osdMinWidth", px(268)))
    readonly property int minWidthText: Math.round(tnum("size.osdMinWidthText", px(150)))
    readonly property int barHeight: Math.round(tnum("size.barHeight", px(6)))
    readonly property int segmentCount: Math.round(tnum("level.segments", 16))
    readonly property int segmentGap: Math.round(tnum("level.segmentGap", 3))
    readonly property int segmentHeight: Math.round(tnum("level.segmentHeight", px(14)))
    ...
    readonly property int shadowPad: Math.round(tnum("shadow.pad", 42))
    readonly property int notifyMargin: Math.round(tnum("spacing.screenMargin", 10))
    readonly property int notifySpacing: Math.round(tnum("spacing.cardGap", 8))
    ...
    readonly property color cardBackground: glass(tcolor("card.background", dark ? Qt.rgba(0x16/255, 0x19/255, 0x1d/255, 0.975)
    ...
    readonly property color panelGlass: glass(tcolor("panel.background", dark ? Qt.rgba(0x14/255, 0x17/255, 0x1b/255, 0.93)
                                                 : Qt.rgba(0xf4/255, 0xf5/255, 0xf8/255, 0.92)))

    readonly property int historyWidth: Math.round(tnum("size.centreWidth", px(400)))
    readonly property bool pill: tflag("radius.pill", true)                 // radius = height/2

    /* Distance from the top screen edge. */
    readonly property int osdTopMargin: Math.round(tnum("spacing.osdTopMargin", px(118)))
    ...
    readonly property color background: glass(tcolor("osd.background", dark ? Qt.rgba(0x16/255, 0x19/255, 0x1d/255, 0.84)
    ...
    readonly property color entryCard: tcolor("entry.background", dark ? Qt.rgba(0x28/255, 0x2e/255, 0x36/255, 1.0)
                                           : Qt.rgba(0xff/255, 0xff/255, 0xff/255, 1.0))
    readonly property color entryCardHover: tcolor("entry.backgroundHover", dark ? Qt.rgba(0x33/255, 0x3a/255, 0x43/255, 1.0)
    ...
    readonly property color entryCardEdge: tcolor("entry.edge", dark ? Qt.rgba(1,1,1,0.07) : Qt.rgba(0,0,0,0.09))

    readonly property color solidSurface: tcolor("surface.solid", dark ? Qt.rgba(0x17/255, 0x1a/255, 0x1f/255, 1.0)
    ...
    readonly property color glassEdge: tcolor("glass.edge", dark ? Qt.rgba(1,1,1,0.22) : Qt.rgba(1,1,1,0.55))
    ...
    readonly property color cardSheen: tcolor("card.sheen", dark ? Qt.rgba(1,1,1,0.028) : Qt.rgba(1,1,1,0.5))

    readonly property color glassSheen: tcolor("glass.sheen", dark ? Qt.rgba(1,1,1,0.07) : Qt.rgba(1,1,1,0.55))

    readonly property color edgeOuter: tcolor("edge.outer", dark ? Qt.rgba(1,1,1,0.09) : Qt.rgba(0,0,0,0.10))
    readonly property color edgeInner: tcolor("edge.inner", Qt.rgba(1, 1, 1, 0.05))
    readonly property int edgeWidth: Math.round(tnum("edge.width", 1))

    // ---- KWin background-contrast effect --------------------------------
    readonly property real bgContrast: tnum("blur.contrast", 0.32)
    readonly property real bgIntensity: tnum("blur.intensity", dark ? 0.92 : 1.12)
    ...
    readonly property real bgSaturation: tnum("blur.saturation", 1.80)

    // ---- Content ---------------------------------------------------------
    readonly property color foreground: tcolor("text.primary", dark ? "#f4f6f8" : "#16181c")
    ...
    readonly property color foregroundMuted: tcolor("text.muted", dark ? Qt.rgba(0xf4/255,0xf6/255,0xf8/255,0.88) : Qt.rgba(0x16/255,0x18/255,0x1c/255,0.78))
    ...
    readonly property color foregroundDim: tcolor("text.secondary", dark ? Qt.rgba(0xf4/255,0xf6/255,0xf8/255,0.60) : Qt.rgba(0x16/255,0x18/255,0x1c/255,0.68))
    readonly property color accent: tcolor("accent", Appearance.accent)
    readonly property color critical: tcolor("urgency.critical", "#da4453")
    ...
    readonly property int chipHaloWidth: Math.round(tnum("chip.haloWidth", 5))
    readonly property color trackColor: tcolor("slider.track", dark ? Qt.rgba(1,1,1,0.15) : Qt.rgba(0,0,0,0.13))

    /* Used where a state has no distinct glyph of its own. */
    readonly property real iconDimmedOpacity: tnum("icon.dimmedOpacity", 0.32)

    readonly property string fontFamily: tstr("font.family", "Noto Sans")
    ...
    readonly property int fontSize: Appearance.fontSize > 0
        ? Appearance.fontSize
        : Math.round(tnum("font.size", Math.max(7, Math.round(11 * s))))
    ...
    readonly property color shadowColor: tcolor("shadow.color", "#000000")
    readonly property real shadowBlur: tnum("shadow.spread", 1.0)
    ...
    readonly property int shadowBlurMax: Math.round(tnum("shadow.blur", 40))
    readonly property real shadowOpacity: tnum("shadow.opacity", dark ? 0.48 : 0.22)
